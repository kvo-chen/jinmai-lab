...
const commercialApplications = [
  {
    ...
    title: '国潮新风尚',
    creator: '设计师小明',
    thumbnail: 'https://space.coze.cn/api/coze_space/gen_image?image_size=square&prompt=Modern%20Chinese%20style%20fashion%20design&sign=63905733bdd62fcca474554620faa6dc',
    brand: '老字号品牌A',
    status: '洽谈中',
  ...
  },
  {
    ...
    title: '传统纹样创新',
    creator: '创意总监小李',
    thumbnail: 'https://space.coze.cn/api/coze_space/gen_image?image_size=square&prompt=Traditional%20Chinese%20patterns%20with%20modern%20twist&sign=7735e995ec7fc16602117334b18c1bf8',
    brand: '老字号品牌B',
    status: '已采纳',
  ...
  },
  {
    ...
    title: '老字号品牌焕新',
    creator: '品牌设计师老王',
    thumbnail: 'https://space.coze.cn/api/coze_space/gen_image?image_size=square&prompt=Traditional%20brand%20modernization%20design&sign=f4ca244274aa44ee2fd865b38f46da78',
    brand: '老字号品牌C',
    status: '已拒绝',
...
export default function Admin() {
  ...
  const [usersData, setUsersData] = useState<Array<any>>([
    { id: 'u1', username: '设计师小明', email: 'user@example.com', role: 'creator', status: 'active' },
    { id: 'u2', username: '文化传播者小李', email: 'li@example.com', role: 'creator', status: 'active' },
    { id: 'u3', username: '品牌方A', email: 'brandA@example.com', role: 'brand', status: 'inactive' }
  ]);
  const [adoptions, setAdoptions] = useState<Array<any>>([
    { id: 'a1', title: '国潮插画应用', brand: '老字号品牌A', status: 'pending', date: '2025-11-10' },
    { id: 'a2', title: '传统纹样商用', brand: '老字号品牌B', status: 'approved', date: '2025-11-09' },
    { id: 'a3', title: '品牌视觉焕新', brand: '老字号品牌C', status: 'rejected', date: '2025-11-08' }
  ]);
  ...
  useEffect(() => {
    const loadAnalytics = async () => {
      ...
      try {
        const res = await apiClient.get<{ analytics: any }>(`/analytics/platform?timeframe=${analyticsTimeframe}`);
    ...
    <div className={`min-h-screen flex ${isDark ? 'bg-gray-900 text-white' : 'bg-gray-50 text-gray-900'}`}>
      ...
      <main className="ml-64 flex-1 p-8">
        ...
        {activeTab === 'analytics' && (
          ...
          >
            <div className="flex justify-between items-center mb-6">
              <h2 className="text-xl font-bold">平台数据分析</h2>
              <select
            ...
            {analyticsLoading ? (
              ...
              <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
                <div className={`lg:col-span-2 p-6 rounded-xl ${isDark ? 'bg-gray-700' : 'bg-gray-50'}`}>
                  <div className="flex justify-between items-center mb-4">
                    <h3 className="font-medium">浏览与点赞趋势</h3>
                  </div>
                  <div className="h-72">
                    <ResponsiveContainer width="100%" height="100%">
                      <LineChart data={(platformAnalytics?.trends?.views || []).map((v: any, i: number) => ({ name: v.date, views: v.count, likes: platformAnalytics?.trends?.likes?.[i]?.count || 0 }))}>
                        <CartesianGrid strokeDasharray="3 3" stroke={isDark ? '#374151' : '#e5e7eb'} />
                ...
                <div className={`p-6 rounded-xl ${isDark ? 'bg-gray-700' : 'bg-gray-50'}`}>
                  <h3 className="font-medium mb-4">总体统计</h3>
                  <div className="space-y-3 text-sm">
                    <div className="flex justify-between"><span>用户数</span><span>{platformAnalytics?.stats?.totalUsers ?? '-'}</span></div>
                    <div className="flex justify-between"><span>作品数</span><span>{platformAnalytics?.stats?.totalWorks ?? '-'}</span></div>
                    <div className="flex justify-between"><span>总浏览</span><span>{platformAnalytics?.stats?.totalViews ?? '-'}</span></div>
                    <div className="flex justify-between"><span>总点赞</span><span>{platformAnalytics?.stats?.totalLikes ?? '-'}</span></div>
                    <div className="flex justify-between"><span>总评论</span><span>{platformAnalytics?.stats?.totalComments ?? '-'}</span></div>
                  </div>
                </div>
              </div>
            )}
        ...
        {activeTab === 'adoption' && (
          ...
          >
            <div className="flex justify-between items-center mb-6">
              <h2 className="text-xl font-bold">战略采纳</h2>
              <button className="bg-red-600 hover:bg-red-700 text-white px-4 py-2 rounded-lg transition-colors">新建采纳</button>
            </div>
                  ...
                  {adoptions.map((item) => (
                    <tr key={item.id} className="hover:bg-gray-700/50">
                      ...
                      <td className="px-4 py-3 text-sm">
                        <span className={`px-2 py-1 rounded-full text-xs ${item.status === 'approved' ? 'bg-green-100 text-green-600' : item.status === 'rejected' ? 'bg-red-100 text-red-600' : 'bg-yellow-100 text-yellow-600'}`}>{item.status === 'approved' ? '已采纳' : item.status === 'rejected' ? '已拒绝' : '待处理'}</span>
                      </td>
                      <td className="px-4 py-3 text-sm">
                        <div className="flex space-x-2">
                          <button className={`px-2 py-1 rounded ${isDark ? 'bg-gray-700 hover:bg-gray-600' : 'bg-gray-100 hover:bg-gray-200'}`} onClick={() => { setAdoptions((prev) => prev.map((a) => a.id === item.id ? { ...a, status: 'approved' } : a)); toast.success('已采纳'); }}>采纳</button>
                          <button className={`px-2 py-1 rounded ${isDark ? 'bg-gray-700 hover:bg-gray-600' : 'bg-gray-100 hover:bg-gray-200'}`} onClick={() => { setAdoptions((prev) => prev.map((a) => a.id === item.id ? { ...a, status: 'rejected' } : a)); toast.success('已拒绝'); }}>拒绝</button>
                        </div>
                      </td>
                    </tr>
                  ))}
            ...
            <div className="flex justify-between items-center mb-6">
              ...
              <div className="flex space-x-2">
                <div className={`relative ${isDark ? 'bg-gray-700' : 'bg-gray-100'} rounded-full px-4 py-1.5`}>
                  <input type="text" placeholder="搜索用户..." className="bg-transparent border-none outline-none w-40 text-sm" />
                ...
                <tbody className="divide-y divide-gray-700">
                  {usersData.map((u) => (
                    <tr key={u.id} className="hover:bg-gray-700/50">
                      ...
                      <td className="px-4 py-3 text-sm">{u.role === 'brand' ? '品牌方' : '创作者'}</td>
                      <td className="px-4 py-3 text-sm">
                        <span className={`px-2 py-1 rounded-full text-xs ${u.status === 'active' ? 'bg-green-100 text-green-600' : 'bg-gray-100 text-gray-600'}`}>{u.status === 'active' ? '活跃' : '禁用'}</span>
                      </td>
                      <td className="px-4 py-3 text-sm">
                        <div className="flex space-x-2">
                          <button className={`px-2 py-1 rounded ${isDark ? 'bg-gray-700 hover:bg-gray-600' : 'bg-gray-100 hover:bg-gray-200'}`} onClick={() => { setUsersData((prev) => prev.map((x) => x.id === u.id ? { ...x, status: x.status === 'active' ? 'inactive' : 'active' } : x)); toast.success(u.status === 'active' ? '已禁用' : '已启用'); }}>{u.status === 'active' ? '禁用' : '启用'}</button>
                          <button className={`px-2 py-1 rounded ${isDark ? 'bg-gray-700 hover:bg-gray-600' : 'bg-gray-100 hover:bg-gray-200'}`} onClick={() => toast.success('已重置密码')}>重置密码</button>
                        </div>
                      </td>
                    </tr>
                  ))}
          ...
          >
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
              <div className={`p-4 rounded-xl ${isDark ? 'bg-gray-700' : 'bg-gray-50'}`}>
                <div className="flex items-center justify-between">
                  <div>
                    <p className="font-medium">开发模式埋点</p>
                    <p className={`text-sm ${isDark ? 'text-gray-400' : 'text-gray-600'}`}>启用后开发环境会上报埋点</p>
                  </div>
                  <button className={`px-3 py-1 rounded ${isDark ? 'bg-gray-600 hover:bg-gray-500' : 'bg-gray-100 hover:bg-gray-200'}`} onClick={() => { const enabled = localStorage.getItem('analytics_dev_enable') === 'true'; if (enabled) { localStorage.removeItem('analytics_dev_enable'); toast.success('开发埋点已关闭'); } else { localStorage.setItem('analytics_dev_enable', 'true'); toast.success('开发埋点已开启'); } }}>
                    {localStorage.getItem('analytics_dev_enable') === 'true' ? '关闭' : '开启'}
                  </button>
                </div>
              </div>
              <div className={`p-4 rounded-xl ${isDark ? 'bg-gray-700' : 'bg-gray-50'}`}>
                <div className="flex items-center justify-between">
                  <div>
                    <p className="font-medium">用户埋点退出</p>
                    <p className={`text-sm ${isDark ? 'text-gray-400' : 'text-gray-600'}`}>退出后不会收集分析数据</p>
                  </div>
                  <button className={`px-3 py-1 rounded ${isDark ? 'bg-gray-600 hover:bg-gray-500' : 'bg-gray-100 hover:bg-gray-200'}`} onClick={() => { const optOut = localStorage.getItem('analytics_opt_out') === 'true'; if (optOut) { localStorage.removeItem('analytics_opt_out'); toast.success('已恢复收集'); } else { localStorage.setItem('analytics_opt_out', 'true'); toast.success('已退出收集'); } }}>
                    {localStorage.getItem('analytics_opt_out') === 'true' ? '恢复' : '退出'}
                  </button>
                </div>
              </div>
            </div>
              ...
              <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                {[
                  { 
                    ...
                    thumbnail: 'https://space.coze.cn/api/coze_space/gen_image?image_size=landscape_4_3&prompt=Chinese%20New%20Year%20event%20template%20design&sign=81643959047243ad6975affdb1151785'
                  },
                  { 
                    id: 2, 
                    title: '品牌联名活动', 
                    description: '为老字号品牌定制联名创作活动',
                    thumbnail: 'https://space.coze.cn/api/coze_space/gen_image?image_size=landscape_4_3&prompt=Brand%20collaboration%20event%20template&sign=ba167ac761dcbcfd21f42dd317d4cae8'
                  },
                  { 
                    id: 3, 
                    title: '文化元素专项', 
                    description: '围绕特定文化元素开展创作征集',
                    thumbnail: 'https://space.coze.cn/api/coze_space/gen_image?image_size=landscape_4_3&prompt=Traditional%20Chinese%20elements%20design%20template&sign=b141e17a22dd8ebfee148a7493093f13'
                  },
                ].map((template) => (
          ...
          >
            ...
            <div className={`overflow-x-auto rounded-xl border ${isDark ? 'border-gray-700' : 'border-gray-200'} mb-8`}>
              <table className="min-w-full">
                ...
                <tbody className="divide-y divide-gray-700">
                  {[
                    { id: 1, name: '设计师小明', avatar: 'https://space.coze.cn/api/coze_space/gen_image?image_size=square&prompt=User%20avatar%20xiaoming&sign=cc76aace202a78fcb07391c53cf45642', level: 'master', works: 45, likes: 1245, adopted: 12, status: 'active' },
                    { id: 2, name: '创意总监小李', avatar: 'https://space.coze.cn/api/coze_space/gen_image?image_size=square&prompt=User%20avatar%20xiaoli&sign=0a161e92e8233ac1e907c1757a08b793', level: 'advanced', works: 32, likes: 876, adopted: 8, status: 'active' },
                    { id: 3, name: '品牌设计师老王', avatar: 'https://space.coze.cn/api/coze_space/gen_image?image_size=square&prompt=User%20avatar%20laowang&sign=ad224bf888bce01af0b2109dca2059e7', level: 'beginner', works: 12, likes: 342, adopted: 2, status: 'active' },
                    { id: 4, name: '插画师小陈', avatar: 'https://space.coze.cn/api/coze_space/gen_image?image_size=square&prompt=User%20avatar%20xiaochen&sign=35e4cf6552a75145358bdcdd1fed4cf1', level: 'master', works: 56, likes: 1890, adopted: 15, status: 'active' },
                    { id: 5, name: '数字艺术家小张', avatar: 'https://space.coze.cn/api/coze_space/gen_image?image_size=square&prompt=User%20avatar%20xiaozhang&sign=24de79776063118a09f9d728513bcc6d', level: 'advanced', works: 28, likes: 765, adopted: 7, status: 'inactive' },
                  ].map((creator) => (
